...
import {
  ...
  numericMatches,
  orderShapes,
} from "../src/utils/inventoryRules.js";

const inventory = await readFile(
  new URL("../src/modules/inventory/Inventory.jsx", import.meta.url),
  "utf8",
);
const styles = await readFile(
  new URL("../src/modules/inventory/inventory.css", import.meta.url),
  "utf8",
);
...
const requireTrue = (condition, label) => {
...
requireText(
  indexStyles,
  'html[data-theme="dark"] .inventory-field select',
  "dark-theme Inventory select styling",
);

// Data contracts.
requireTrue(formatDecimal(5) === "5.000", "three-decimal display format");
for (const query of ["21", "21.", "21.4", "21.45", "21.450"])
  requireTrue(numericMatches("21.450", query), `numeric search for ${query}`);
requireTrue(
  inventoryMatchesSearch({ size: "21.450", weight: 99 }, "21.4mm"),
  "mm Size-only search",
);
requireTrue(
...
requireTrue(
  ordered.join("|") === "Pan|Oval|Future Shape|Round",
  "shape order with Round last",
);
