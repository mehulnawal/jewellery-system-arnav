...
const SUPERADMIN_PROFILE = {
  accessId: "granthaexports@gmail.com",
  role: "superadmin",
  allowedModules: [
    "inventory",
    "purchase",
...
const resolveProfile = async (current) => {
  ...
  if (!profile.exists() && current.uid === SUPERADMIN_UID) {
    await setDoc(doc(db, "employeeProfiles", current.uid), {
      uid: current.uid,
      ...SUPERADMIN_PROFILE,
...
export function AuthProvider({ children }) {
  ...
  const activateProfile = async (profile) => {
    setUser(profile);
    try {
      await captureFirstLogin(profile);
    } catch (error) {
      console.warn("First-login audit could not be saved.", error);
    }
  };
  const loginEmployee = async (accessId, password) => {
    const credential = await signInWithEmailAndPassword(
      auth,
      accountEmail(accessId),
      password,
    );
    const profile = await resolveProfile(credential.user);
    if (!profile.exists() || !profile.data().active) {
      await signOut(auth);
    ...
    () =>
      onAuthStateChanged(auth, async (current) => {
        ...
        if (
          !profile.exists() ||
          (!data.active &&
            (!timestampMs(data.accessRevokedAt) || deadline <= Date.now()))
        ) {
          await signOut(auth);
          setUser(null);
          setLoading(false);
        ...
        if (!data.active) {
          expiryAt.current = deadline;
          setRemaining(Math.max(0, deadline - Date.now()));
        }
        await activateProfile(data);
        setLoading(false);
      }),
    [],
  );
  useEffect(() => {
    ...
    return onSnapshot(doc(db, "employeeProfiles", user.uid), (snapshot) => {
      ...
      if (!revokedAt || deadline <= Date.now()) {
        void logout();
        return;
      }
      expiryAt.current = deadline;
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      const next = Math.max(0, expiryAt.current - Date.now());
      setRemaining(next);
      if (!next) void logout();
    }, 1000);
  ...
  return (
    <AuthContext.Provider value={value}>
      {children}
      {remaining > 0 && <RevocationNotice milliseconds={remaining} />}
    </AuthContext.Provider>
  );
}
function RevocationNotice({ milliseconds }) {
  const seconds = Math.ceil(milliseconds / 1000),
    value = `${String(Math.floor(seconds / 60)).padStart(2, "0")}:${String(seconds % 60).padStart(2, "0")}`;
  return (
    <div className="access-revocation-notice" role="alert">
      <b>Your account access has been revoked.</b>
      <span>Please save your current work.</span>
      <strong>Automatic logout in {value}</strong>
    </div>
  );
}
