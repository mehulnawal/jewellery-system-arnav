...
export async function captureFirstLogin(user) {
  ...
  const ref = doc(
    ...
    `${encodeURIComponent(actor.uid)}_${day}`,
  );
  const now = Date.now();
...
const inventorySnapshot = (item, origin) => ({
  ...
  size: item.size ?? "",
  box: item.box ?? "",
  origin: origin || item.origin || "Manual",
});
const changedFields = (before, after) =>
  Object.keys(after).reduce((result, field) => {
    if (
...
export async function writeInventoryActivity(
  action,
  item,
  { before, origin, user } = {},
) {
  ...
  const event = {
    ...
    ...snapshot,
    actor: login.actor,
    accessIdSnapshot: login.actor.accessId,
    firstLoginAtMs: login.firstLoginAtMs,
    eventAtMs: Date.now(),
    eventDate: dateKey(),
    createdAt: serverTimestamp(),
  };
  if (action === "edited")
    event.changes = changedFields(inventorySnapshot(before), snapshot);
