...
export const generateAccessId = () => `emp${Math.random().toString(36).slice(2, 8)}`
...
export async function createEmployee({ accessId, password, permissions }) {
  ...
  if (!/^[A-Za-z0-9_-]{3,32}$/.test(normalized)) throw new Error('Access ID must be 3-32 letters, numbers, hyphens, or underscores.')
